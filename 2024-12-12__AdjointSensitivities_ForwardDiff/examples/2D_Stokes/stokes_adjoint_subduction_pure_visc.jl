...
@views function Res!(Fvec::AbstractVector{T}, U::Vector{<:AbstractArray{T}}, Δ::NTuple, N::NTuple, BC::NamedTuple, Params::NamedTuple, rheology, Δt) where T<:Number
    ...
    Fforce2[:,2:end-1]  =   diff(τxz[:,2:end-1], dims=1)./dx + diff(-Plocal + τzz, dims=2)./dz - g*(Params.ρ[:,2:end] + Params.ρ[:,1:end-1])/2 
...
@views function Res_adjoint!(
    ...
    Fforce1[2:end-1,:]  = diff(-Plocal + τxx, dims=1)./dx + diff(τxz[2:end-1,:], dims=2)./dz
    Fforce1[1,:]        = Vx[2,:]       #.- BC.εxx_bg*Params.x[1];
    Fforce1[end,:]      = Vx[end-1,:]     #.- BC.εxx_bg*Params.x[end];

    # z-momentum balance
    Fforce2             =   zeros(eltype(ρ), size(Vz))
    g                   =   rheology[1].Gravity[1].g.val;
    Fforce2[:,2:end-1]  =   diff(τxz[:,2:end-1], dims=1)./dx + diff(-Plocal + τzz, dims=2)./dz - g*(ρ[:,2:end] + ρ[:,1:end-1])/2 
    Fforce2[:,      1]  =   Vz[:,2]     #.- BC.εzz_bg*Params.z[1];
    Fforce2[:,    end]  =   Vz[:,end-1]   #.- BC.εzz_bg*Params.z[end];
...
function adjoint_sensitivities(J_forward, U; IDX=100, IDX1=nothing)
    ...
    # λ_Vx = reshape(λ[1:(Nx+1)*Nz], N[1])
    # λ_Vx[1,:] .= 0
    # λ_Vx[end,:] .= 0
    # λ_Vx[:,1] .= 0
    # λ_Vx[:,end] .= 0

    # λ_Vz = reshape(λ[(Nx+1)*Nz+1:((Nx+1)*Nz+(Nx*(Nz+1)))], N[2])
    # λ_Vz[:,1] .= 0
    # λ_Vz[:,end] .= 0
    # λ_Vz[1,:] .= 0
    # λ_Vz[end,:] .= 0

    # λ_P  = reshape(λ[((Nx+1)*Nz+(Nx*(Nz+1)))+1:end], N[3])
...
η   =    ones(Nx,Nz) .* 1e0             # viscosity @ center
...
ind = findall((XC .> 1.15) .& (ZC .> -0.2))
phasec[ind] .= 2
ρ[ind] .= 1.5
η[ind] .= 1e3
K[ind] .= 1e-6

ind = findall((XV .> 1.15) .& (ZV .> -0.2))
phasev[ind] .= 2
ηv[ind] .= 1e3

indices = Matrix{Bool}(undef, Nx, Nz)
polyX   = [0.75     ;  1.05 ; 1.3   ;   1.15]
polyZ   = [ - 0.5  ;  -0.5 ; -0.2  ;   -0.0]
inpolygon!(indices, polyX, polyZ, XC, ZC)
phasec[indices] .= 2
ρ[indices] .= 1.5
η[indices] .= 1e3
K[indices] .= 1e-6

indices = Matrix{Bool}(undef, Nx+1, Nz+1)
inpolygon!(indices, polyX, polyZ, XV, ZV)
phasev[indices] .= 2
ηv[indices] .= 1e3
...
rheology = (
        SetMaterialParams(;
            ...
            Phase             = 1,
            Density           = ConstantDensity(ρ=1.2kg/m^3),
...
IDX = []
for i in Nx*(Nz-20)-round(Int,7*Nx/10):Nx*(Nz-20)-round(Int,3*Nx/10)
    IDX_plot = findall(IDX_array .== i)
    push!(IDX, IDX_plot)
end

#plot same sections as Georg
ind_box = findall(XC .> 1.9 .|| XC .< 0.1 .|| ZC .< -0.8 .|| ZC .> -0.2)
...
# #sensitivities
# ax1 = Axis(fig[2,3], xlabel = "x [km]", ylabel = "z [km]", title = "sensitivites ρ ")
# hm1 = heatmap!(ax1, xc*CharUnits.length.val, zc[1:end-32]*CharUnits.length.val, sens_ρ[:,1:end-32], colormap = :ocean, colorrange = (-0.5*maximum(abs.(sens_ρ[:,1:end-32])), 0.5*maximum(abs.(sens_ρ[:,1:end-32]))))
# # scatter!(ax1, [xc[IDX[1][1]]*CharUnits.length.val], [zc[IDX[1][2]]*CharUnits.length.val], markersize = 5, color = :red)
# Colorbar(fig[2,4], hm1, label = "sensitivites")

# ax4 = Axis(fig[3,1], xlabel = "x [km]", ylabel = "z [km]", title = "sensitivites η")
# hm4 = heatmap!(ax4, xc*CharUnits.length.val, zc[1:end-32]*CharUnits.length.val, sens_η[:,1:end-32], colorrange = (-0.25*maximum(abs.(sens_η[:,1:end-32])), 0.25*maximum(abs.(sens_η[:,1:end-32]))), colormap = :ocean)
# # scatter!(ax4, [xc[IDX[1][1]]*CharUnits.length.val], [zc[IDX[1][2]]*CharUnits.length.val], markersize = 5, color = :red)
# Colorbar(fig[3, 2], hm4, label = "sensitivites")

# ax5 = Axis(fig[3,3], xlabel = "x [km]", ylabel = "z [km]", title = "sensitivities ρ + η")
# hm5 = heatmap!(ax5, xc*CharUnits.length.val, zc*CharUnits.length.val, sensitivities[1] + average_2D(sensitivities[2]), colormap= :ocean, colorrange = (-maximum(abs.(sensitivities[1] + average_2D(sensitivities[2]))), maximum(abs.(sensitivities[1] + average_2D(sensitivities[2])))))
...
# fig = Figure()
# ax = Axis(fig[1, 1], xlabel = "x [km]", ylabel = "z [km]", title = "λ_Vx")
# hm = heatmap!(ax, xc*CharUnits.length.val, zc*CharUnits.length.val, reshape(λ[1:(Nx+1)*Nz], (Nx+1,Nz)), colormap = :viridis#=, colorrange = (-maximum(abs.(λ[1:81*80])), maximum(abs.(λ[1:81*80])))=#)
# Colorbar(fig[1, 2], hm, label = "λ_Vx")

# ax1 = Axis(fig[2, 1], xlabel = "x [km]", ylabel = "z [km]", title = "λ_Vz")
# hm1 = heatmap!(ax1, xc*CharUnits.length.val, zc*CharUnits.length.val, reshape(λ[(Nx+1)*Nz+1:((Nx+1)*Nz+(Nx*(Nz+1)))], (Nx,Nz+1)), colormap = :viridis#=, colorrange = (-maximum(abs.(λ[80*81+1:2*80*81])), maximum(abs.(λ[80*81+1:2*80*81])))=#)
# Colorbar(fig[2, 2], hm1, label = "λ_Vz")

# ax2 = Axis(fig[3, 1], xlabel = "x [km]", ylabel = "z [km]", title = "λ_P")
# hm2 = heatmap!(ax2, xc*CharUnits.length.val, zc*CharUnits.length.val, reshape(λ[((Nx+1)*Nz+(Nx*(Nz+1)))+1:end], (Nx,Nz)), colormap = :viridis#=, colorrange = (-maximum(abs.(λ[2*80*81+1:end])), maximum(abs.(λ[2*80*81+1:end])))=#)
# Colorbar(fig[3, 2], hm2, label = "λ_P")
# display(fig)


# using DelimitedFiles

# Vx = readdlm("/Users/jacob/Downloads/Vx.csv", ',')
# Vz = readdlm("/Users/jacob/Downloads/Vy.csv", ',')
# P = readdlm("/Users/jacob/Downloads/P.csv", ',')

# deltaVx = Vx[2:end,:] - Usol[1]
# deltaVz = Vz[2:end,:] - Usol[2]
# deltaP = P[2:end,:] - Usol[3]

# Usol_chris = [Float64.(Vx[2:end,:]), Float64.(Vz[2:end,:]), Float64.(P[2:end,:])]
# sensitivities, λ, J_adjoint = adjoint_sensitivities(J2, Usol_chris, IDX=Nx*(Nz-1)-round(Int,7*Nx/10):Nx*(Nz-1)-round(Int,3*Nx/10))



# fig = Figure()
# ax1 = Axis(fig[1,1], xlabel = "x [km]", ylabel = "z [km]", title = "Vx Chris")
# ax2 = Axis(fig[1,3], xlabel = "x [km]", ylabel = "z [km]", title = "Vz Chris")
# ax3 = Axis(fig[1,5], xlabel = "x [km]", ylabel = "z [km]", title = "P Chris")
# hm = heatmap!(ax1, x*CharUnits.length.val, zc*CharUnits.length.val, Float64.(Vx[2:end,:]), colormap = :viridis)
# hm2 = heatmap!(ax2, x*CharUnits.length.val, zc*CharUnits.length.val, Float64.(Vz[2:end,:]), colormap = :viridis)
# hm3 = heatmap!(ax3, x*CharUnits.length.val, zc*CharUnits.length.val, Float64.(P[2:end,:]), colormap = :viridis)
# Colorbar(fig[1, 2], hm, label = "Vx")
# Colorbar(fig[1, 4], hm2, label = "Vz")
# Colorbar(fig[1,6], hm3, label = "P")

# ax4 = Axis(fig[2,1], xlabel = "x [km]", ylabel = "z [km]", title = "delta Vx")
# ax5 = Axis(fig[2,3], xlabel = "x [km]", ylabel = "z [km]", title = "delta Vz")
# ax6 = Axis(fig[2,5], xlabel = "x [km]", ylabel = "z [km]", title = "delta P")
# hm4 = heatmap!(ax4, xc*CharUnits.length.val, zc*CharUnits.length.val, deltaVx, colormap = :viridis)
# hm5 = heatmap!(ax5, xc*CharUnits.length.val, zc*CharUnits.length.val, deltaVz, colormap = :viridis)
# hm6 = heatmap!(ax6, xc*CharUnits.length.val, zc*CharUnits.length.val, deltaP, colormap = :viridis)
# Colorbar(fig[2, 2], hm4, label = "delta Vx")
# Colorbar(fig[2, 4], hm5, label = "delta Vz")
# Colorbar(fig[2,6], hm6, label = "delta P")

# ax5 = Axis(fig[3,1], xlabel = "x [km]", ylabel = "z [km]", title = "Vx Jacob")
# hm5 = heatmap!(ax5, xc*CharUnits.length.val, zc*CharUnits.length.val, Usol[1]*CharUnits.velocity.val, colormap = :viridis)
# Colorbar(fig[3, 2], hm5, label = "Vx Jacob")
# ax6 = Axis(fig[3,3], xlabel = "x [km]", ylabel = "z [km]", title = "Vz Jacob")
# hm6 = heatmap!(ax6, xc*CharUnits.length.val, zc*CharUnits.length.val, Usol[2]*CharUnits.velocity.val, colormap = :viridis)
# Colorbar(fig[3, 4], hm6, label = "Vz Jacob")
# ax7 = Axis(fig[3,5], xlabel = "x [km]", ylabel = "z [km]", title = "P Jacob")
# hm7 = heatmap!(ax7, xc*CharUnits.length.val, zc*CharUnits.length.val, data, colormap = :viridis)
# Colorbar(fig[3, 6], hm7, label = "P Jacob")
# display(fig)

fig = Figure(size=(1000, 1200))

ax1 = Axis(fig[1,1], xlabel = "x [km]", ylabel = "z [km]", title = "Phases")
hm  = heatmap!(ax1, xc*CharUnits.length.val, zc[1:end-32]*CharUnits.length.val, phasec[:,1:end-32], colormap = :viridis)
Colorbar(fig[1,2], hm, label = "Phase")

ax1 = Axis(fig[2,1], xlabel = "x [km]", ylabel = "z [km]", title = "sensitivites ρ ")
hm1 = heatmap!(ax1, xc*CharUnits.length.val, zc[1:end-32]*CharUnits.length.val, sens_ρ[:,1:end-32], colormap = :ocean, colorrange = (-0.56*maximum(abs.(sens_ρ[:,1:end-32])), 0.9*maximum(abs.(sens_ρ[:,1:end-32]))))
# scatter!(ax1, [xc[IDX[1][1]]*CharUnits.length.val], [zc[IDX[1][2]]*CharUnits.length.val], markersize = 5, color = :red)
Colorbar(fig[2,2], hm1, label = "sensitivites")

ax4 = Axis(fig[3,1], xlabel = "x [km]", ylabel = "z [km]", title = "sensitivites η")
hm4 = heatmap!(ax4, xc*CharUnits.length.val, zc[1:end-32]*CharUnits.length.val, sens_η[:,1:end-32], colorrange = (-0.5*maximum(abs.(sens_η[:,1:end-32])), 0.9*maximum(abs.(sens_η[:,1:end-32]))), colormap = :ocean)
# scatter!(ax4, [xc[IDX[1][1]]*CharUnits.length.val], [zc[IDX[1][2]]*CharUnits.length.val], markersize = 5, color = :red)
Colorbar(fig[3, 2], hm4, label = "sensitivites")
