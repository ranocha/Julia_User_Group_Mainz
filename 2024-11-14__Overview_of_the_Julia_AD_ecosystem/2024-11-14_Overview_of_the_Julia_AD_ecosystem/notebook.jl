...
# ╔═╡ ff3e2520-cf49-41ad-b39e-9b93454fb62c
begin
	using PlutoUI
...
# ╔═╡ c8597eb3-8e7c-4a6a-8334-2f490e80cb40
begin
	using CairoMakie
...
# ╔═╡ aec53114-def1-4dbf-b651-f41764d4a326
using Symbolics

# ╔═╡ c9c6ae26-2e46-4113-bdcd-b3a04c6b23f0
using Checkpointing

# ╔═╡ 0d964d8a-a1cc-11ef-109a-355df4ce1d5b
md"""
# Overview of the Julia AD ecosystem
*Julia User Group -- 2024-11-14*

Last week Hendrik introduced AD to us using a custom data-type `MyDual`.
"""

# ╔═╡ 57b5a9da-1753-476b-822f-9245f84957f3
begin
...
# ╔═╡ c4a5f9a7-4e7d-4245-b6cb-64bf46f114f7
...
# ╔═╡ b06c0fa4-c8b4-4119-97c3-b64890564249
Babylonian(100.0)

# ╔═╡ 001ed31c-397b-4bb0-858d-69b1835ff775
Babylonian(MyDual(100.0, 1.0), N=10)

# ╔═╡ 8881a438-fcaa-4fce-91bd-cdb176c87b72
let 
	@variables x
	Symbolics.derivative(Babylonian(x, N=4), x; simplify = false)
end

# ╔═╡ 339bc99a-3c59-4c25-bce8-2964a20641b9
let 
	@variables x
	substitute(
		Symbolics.derivative(Babylonian(x, N=10), x; simplify = false),
		Dict(x => 100.0)
	)
end

# ╔═╡ 59289961-9ee7-4ecd-88c2-68f43d581461
# using SymPy

# ╔═╡ 73e4c26f-a722-49c0-b5b8-a4c98daf14e6
# let 
# 	SymPy.@syms x
# 	SymPy.diff(Babylonian(x, N=2), x)
# end

# ╔═╡ ba2c4083-c60a-40c6-a64f-53d5a041eaa9
# let 
# 	SymPy.@syms x
# 	SymPy.diff(Babylonian(x, N=10), x)(100.0)
# end

# ╔═╡ a321d433-b07d-44a5-9558-129d0a50a510
md"""
## Pitfalls of AD

1. Where does this approach fail
2. Brief introduction to Enzyme
"""

# ╔═╡ 1efbc3f7-f622-449d-96ba-86a521ad9ada
md"""
### Operator overload approaches

1. Fail on overly tight type-constraints
2. Mixing types
3. Pertubation-confusion
...
# ╔═╡ 20be835a-bda9-4388-8260-8991f59f02a2
f(MyDual(2.0, 1.0))
...
# ╔═╡ 97497d1f-b03b-4c2b-9638-510465b17154
g(MyDual(2.0, 1.0))

# ╔═╡ 330b23f7-e03a-42f4-b665-7450821a69b0
ForwardDiff.derivative(g, 2.0)

# ╔═╡ bdf383c4-718c-4cce-a6bc-c02cf97e0f84
md"""
...
# ╔═╡ 088f0afd-ecb5-4750-b586-681eac1d9350
O = let 
	@variables x, y
	D_x = Symbolics.Differential(x)
	D_y = Symbolics.Differential(y)
	D_x(x*D_y(x+y))
end

# ╔═╡ 876e0c57-9904-4008-a8fd-ac0cbc1ec11a
expand_derivatives(O)

# ╔═╡ 670107ff-d32e-4e31-8840-67e47fa0e85b
let 
	D = ForwardDiff.derivative
	D(x -> x * D(y -> x + y, 1), 1) # Should be 1
end

# ╔═╡ b115ea72-e13c-48eb-98ad-1c91d95b178d
let
	D(f, x) = (f(MyDual(x, one(x)))::MyDual).deriv
	D(x -> x * D(y -> x + y, 1), 1) # Should be 1
end

# ╔═╡ 7dd64f86-e4b6-42de-9272-df8c3c688ef0
md"""
Solution to these problems is "compiler-based"/"source-to-source" AD

## Enzyme
"""

# ╔═╡ 39fd3dd0-3e05-45c9-a4c2-d8fd4ec602d0
Enzyme.gradient(Forward, f, 2.0)

# ╔═╡ 59b719a7-7edf-458e-92d6-9b848861ce06
Enzyme.gradient(Forward, g, 2.0)

# ╔═╡ 60685aab-5ab5-459a-976b-814a141384fe
let
	D(f, x) = Enzyme.gradient(Forward, f, x)[1]
	D(x -> x * D(y -> x + y, 1.0), 1.0) # Should be 1
end

# ╔═╡ 39df497b-82c8-4219-924e-0742eea51ec4
md"""
### Directional derivative
"""

# ╔═╡ 178e6fb3-c9b8-44bd-9806-758e98a46906
F(x, y, z) = (x*y*z, x*y/z)

# ╔═╡ 7b9e3c1b-a593-4d50-83e5-449cab7d3867
autodiff(Forward, F, 
	Duplicated(1.0, 1.0), Duplicated(2.0, 0.0), Duplicated(3.0, 0.0)) |> only

# ╔═╡ 422aa7e2-8a5d-4d73-9584-3d7655f11842
autodiff(Forward, F, 
	Duplicated(1.0, 0.0), Duplicated(2.0, 1.0), Duplicated(3.0, 0.0)) |> only

# ╔═╡ 0301d990-9142-4d89-ab72-583ce707ce41
autodiff(Forward, F, 
	Duplicated(1.0, 0.0), Duplicated(2.0, 0.0), Duplicated(3.0, 1.0)) |> only

# ╔═╡ 6b7c3ab9-460f-4368-8532-b0d8f12279f7
begin
	function G!(Y, X)
		Y[1] = X[1] * X[2] * X[3]
		Y[2] = X[1] * X[2] / X[3]
...
# ╔═╡ 91e9a6e8-2459-420a-bb31-9ee2c008aa8a
begin
	u = [1.0, 2.0, 3.0]
	dir = [1.0, 0.0, 0.0]
end

# ╔═╡ 64412ac5-df42-4a57-8324-799d6e3925ad
autodiff(Forward, G, Duplicated(u, dir)) |> only

# ╔═╡ 381f2979-7fd0-4ba4-8d38-61cb2e361835
J = Enzyme.jacobian(Forward, G, u) |> only

# ╔═╡ e694bea1-e1f9-4bd2-bfb3-501c982e44c7
w = rand(3)

# ╔═╡ 868be1fd-a2d2-4d44-a292-97da72998296
autodiff(Forward, G, Duplicated(u, w)) |> only

# ╔═╡ 69c9e393-87b2-4748-b359-00b37cd152f0
J*w

# ╔═╡ 1e9dac04-6ec7-49b2-8ae7-290ab28f3bb6
v = rand(2) .+ 0.5

# ╔═╡ 45e33145-8f1c-44f5-be60-51ea5d9464d4
let 
	du = zero(u)
	autodiff(Enzyme.Reverse, G!, Duplicated(zeros(2), v), 
						         Duplicated(u, du))
	du
end

# ╔═╡ 97347d5e-a2db-465c-adf1-44c5bde5ade0
J'*v

# ╔═╡ 20afce1f-add3-471f-b9d8-fba37608f94a
w2  = rand(3)

# ╔═╡ 5c21aee8-37e1-4665-82b6-59e817386bee
autodiff(Forward, G, Duplicated(u, w2)) |> only

# ╔═╡ 73a96d2a-3e59-4040-a848-1c187435a736
# Linear combination
only(autodiff(Forward, G, Duplicated(u, w2 ./ norm(w2)))) .* norm(w2)

# ╔═╡ 90c673ad-9627-4dad-ab33-305ea5458559
f2(x, y) = sin(x) * cos(y)

# ╔═╡ a13a1c9d-1260-463a-be4b-3a6773354ec1
begin
	xs = LinRange(0, 2pi,20)
	ys = LinRange(0, 2pi,20)
	zs = [f2(x, y) for x in xs, y in ys]
	nothing
end

# ╔═╡ 757eaa65-f662-4be0-91d2-fd6e6ae1447a
surface(xs, ys, zs,  axis=(type=Axis3,))

# ╔═╡ 46fd8eeb-6b4d-49a5-aec7-5271e1d14701
function df2_dx(p)
	autodiff(Forward, f2, Duplicated(p[1], 1.0), Duplicated(p[2], 0.0)) |> only
end

# ╔═╡ 49c5511d-8dcc-4e95-9d67-9c68fc6dc480
function df2_dy(p)
	autodiff(Forward, f2, Duplicated(p[1], 0.0), Duplicated(p[2], 1.0)) |> only
end

# ╔═╡ d5d6ad3a-6ae3-4972-8678-2066b9cd3db5
function df2(p)
	n = normalize(ones(2))
	autodiff(Forward, f2, Duplicated(p[1], n[1]), Duplicated(p[2], n[2])) |> only
end

# ╔═╡ a446991a-1e96-4b07-958d-efebff9647a5
strength_1 = vec([normalize(df2_dx((x, y))) for x in xs, y in ys])

# ╔═╡ 88b28df8-4365-4629-bc5f-13774ff66561
arrows(xs, ys, df2_dx, arrowsize = 10, lengthscale = 0.3,
       arrowcolor = strength_1, linecolor = strength_1)

# ╔═╡ 112ffff1-2772-415a-a5e7-588139854bbd
strength_2 = vec([normalize(df2_dy((x, y))) for x in xs, y in ys])

# ╔═╡ 7042d2c4-f787-4723-9572-ef04632c3536
arrows(xs, ys, df2_dy, arrowsize = 10, lengthscale = 0.3,
		arrowcolor = strength_2, linecolor = strength_2)

# ╔═╡ 4d6aa7ca-8546-43fd-acaf-d3a93a4cb80e
strength_3 = vec([normalize(df2((x, y))) for x in xs, y in ys])

# ╔═╡ 325edf31-9a2f-45e7-98f6-d1886fac64a3
arrows(xs, ys, df2, arrowsize = 10, lengthscale = 0.3, 
	   arrowcolor = strength_3, linecolor = strength_3)

# ╔═╡ 33f56438-04cc-4b45-a8aa-9fb2c0a3f25d
# begin
# 	f_1((x, y)) = (x+y^2, x+2y)
# 	f_2((x, y)) = (x+y - x^2, 2x+3y)
# 	f_3((x, y)) = (x^2+y^2 - 1/4, x/2+y)
# end

# ╔═╡ e0985f9f-5203-4896-868b-efad53757769
# arrows(xs, ys, f_1,  arrowsize = 10, lengthscale = 0.3)

# ╔═╡ e4bbbb04-4fca-46d3-8b26-38230ecaf064
# arrows(xs, ys, f_2,  arrowsize = 10, lengthscale = 0.3)

# ╔═╡ 52899652-4477-4c7c-ac0b-dd9e5f9e6410
# arrows(xs, ys, f_3,  arrowsize = 10, lengthscale = 0.3)
...
autodiff(Enzyme.Reverse, use_temp, Active(1.0))

# ╔═╡ 8d2f5106-2970-40c8-afd1-f4ae3ac07bcb
autodiff(set_runtime_activity(Enzyme.Reverse), use_temp, Active(1.0)) # incorrect
...
autodiff(Enzyme.Reverse, use_temp2, Active(1.0), Const(zeros(3)))

# ╔═╡ af43c12e-1594-4f20-9341-65458b115e53
autodiff(Enzyme.Reverse, use_temp2, Active(1.0), Duplicated(zeros(2), zeros(2)))
...
# ╔═╡ 63eb995d-1481-4f97-a8c7-57c3471608a2
md"""
and much more, see: https://enzymead.github.io/Enzyme.jl/stable/faq/
"""
...
https://wires.onlinelibrary.wiley.com/doi/full/10.1002/widm.1555
...
# ╔═╡ e71cd79d-97a1-49d7-b326-19217c8eec35
md"""

More information on Enzyme: https://dj4earth.github.io/MPE24/
"""

# ╔═╡ a518a63d-17ae-4496-b048-c61f66f05706
md"""
### Topics not covered
- Checkpointing.jl
- Chainrules/EnzymeRules
- AD through parallelism
"""

# ╔═╡ 89bdc1d7-4904-4977-8cd3-fab94a0c4d79
md"""
## DifferentiationInterface

https://github.com/JuliaDiff/DifferentiationInterface.jl?tab=readme-ov-file#compatibility
"""

# ╔═╡ 098f8395-4d59-4843-8166-3c0aaf4b5e04
begin
	f_or_another(x) = sum(abs2, x)
	x_or_another = [1.0, 2.0]
end

# ╔═╡ c3a808ce-46e1-46af-9bc2-8cb5ce81a5d4
value_and_gradient(f_or_another, AutoForwardDiff(), x_or_another) # returns (5.0, [2.0, 4.0]) with ForwardDiff.jl

# ╔═╡ ac66765b-432e-4223-b2f6-557f38de372c
value_and_gradient(f_or_another, AutoEnzyme(),      x_or_another) # returns (5.0, [2.0, 4.0]) with Enzyme.jl

# ╔═╡ 1ca55367-27af-4e05-8438-c93d797ec30f
value_and_derivative(f, AutoForwardDiff(),  2.0) # Inherits the same limitations

# ╔═╡ 613a9b11-a0e6-4d7d-a822-7403d80ac941
md"""
## Checkpointing
...
# ╔═╡ 59b2706c-ffc4-4a0f-b2b2-0d790b774890
mutable struct Heat
...
# ╔═╡ 29a1592f-c4a5-4867-ac67-282f438932b1
...
# ╔═╡ ee40a519-23e7-45b8-9431-0726893ef560
...
# ╔═╡ a10adfd1-ba5c-4e86-8299-3d1730d9dd3d
function heat(scheme::Scheme, tsteps::Int)
    ...
    # Compute gradient
    autodiff(Enzyme.ReverseWithPrimal, sumheat, Const, Duplicated(heat, dheat), Const(scheme), Const(tsteps))

    return heat.Tnext, dheat.Tnext[2:end-1]
end

# ╔═╡ 33445db1-9dbd-4d9a-a3ac-b8300c114cb6
begin 
	tsteps = 500
	T, dT = heat(Revolve{Heat}(tsteps,4), tsteps)
end

# ╔═╡ 420a0861-db5d-4c42-95d5-9fde775efbe7
# Plot function values
plot(T)


# ╔═╡ 04e9476b-d24c-4996-b840-20c9291dc5f3
...
manifest_format = "2.0"
project_hash = "590a615044f3393e1c10ea929d6351bb6dde32cc"
...
git-tree-sha1 = "4c20133a2c9109e15ed7d973015a108d8a6bd4a5"
